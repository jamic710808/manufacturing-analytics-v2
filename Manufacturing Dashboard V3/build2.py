#!/usr/bin/env python3
"""
Build script for Manufacturing Dashboard V3 - Fixed version.
Uses precise unique markers for all replacements to avoid duplication.
"""
...
# ============================================================
# 1. TITLE & VERSION (unique strings, safe to replace)
# ============================================================
v3 = v2.replace(
...
# ============================================================
# 2. ADDITIONAL CSS - insert before </style> (only 1 occurrence)
# ============================================================
new_css = """
        /* ===== V3 New Tab Styles ===== */
...
# ============================================================
# 3. SIDEBAR: Insert new nav group before closing nav-menu + aside
# ============================================================
new_sidebar = """
            ...
            <a class="nav-item" data-target="tab-exec-cockpit">
                <svg class="nav-icon" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm4.44-12.56l-5.03 5.03-2.12-2.12L8 11.64l3.41 3.41 6.32-6.32-1.29-1.29z"/></svg>
                <span>20. 管理駕駛艙</span>
            </a>
...
# This is a unique anchor - the DAX tab followed by closing div and aside
dax_tab_end = """                <span>14. DAX 知識庫</span>
            </a>
        </div>
    </aside>"""

v3 = v3.replace(dax_tab_end, """                <span>14. DAX 知識庫</span>
            </a>""" + new_sidebar + """        </div>
    </aside>""")

# ============================================================
# 4. TAB PANES: Insert 8 new panes before closing dashboard-container
# ============================================================
new_panes = """
...
# Unique anchor: DAX KB tab pane closing + dashboard container closing
dax_pane_end = """            <!-- Page 14: DAX Knowledge Base -->
            <div class="tab-pane" id="tab-dax-kb">
                <input type="text" class="dax-search" id="dax-search" placeholder="搜尋 DAX 公式（如 CALCULATE、SUMX、庫存...）">
                <div class="dax-tabs" id="dax-tabs"></div>
                <div id="dax-content"></div>
            </div>
        </div>
    </main>"""

v3 = v3.replace(dax_pane_end, """            <!-- Page 14: DAX Knowledge Base -->
            <div class="tab-pane" id="tab-dax-kb">
                <input type="text" class="dax-search" id="dax-search" placeholder="搜尋 DAX 公式（如 CALCULATE、SUMX、庫存...）">
                <div class="dax-tabs" id="dax-tabs"></div>
                <div id="dax-content"></div>
...
# ============================================================
# 5. JAVASCRIPT: Insert V3 data generator + render functions
# The unique anchor is the final initialization block at end of script
# ============================================================

# Find the unique initialization block at the end
init_block = """    // ===== 初始化 =====
    currentData = generateSampleDataV2();
    initDateFilter();
    renderMap['tab-overview']();  // 只渲染首頁
</script>"""

new_js_block = """    // ===== V3 Extended Data Generator =====
    const generateSampleDataV3 = () => {
        ...
        base.ironTriangle = months12.map(m => ({
            month: m, inventoryScore: Math.round(70+Math.random()*20),
            supplierScore: Math.round(75+Math.random()*20), costScore: Math.round(65+Math.random()*25),
        }));

        const totalInv = base.materials.reduce((s,m)=>s+m.stockQty*m.unitCost,0);
        base.cashFlow = {
            lastYearInventory: Math.round(totalInv*0.85), newPurchases: Math.round(totalInv*0.6),
            productionConsumption: Math.round(totalInv*0.45), staleAccumulation: Math.round(totalInv*0.08),
            currentInventory: totalInv, capitalTiedRate: 0.32,
            cashReleasePotential: Math.round(totalInv*0.15), staleAmount: Math.round(totalInv*0.08),
        ...
        base.purchaseOrders.forEach(po => { po.cycleDays = po.actualLeadTime + Math.floor(Math.random()*10); });
        ...
        base.benchmark = {
            ...
            company:[72,88,96,78,65], industryAvg:[65,82,93,70,60], industryBest:[90,97,99,92,85],
        };
        return base;
    };
    ...
    renderMap['tab-iron-triangle'] = () => {
        ...
        const bgFill = isDark ? '#1e293b' : '#fff8ee';
        const scoreColor = s => s>=85 ? '#10b981' : s>=70 ? '#f59e0b' : '#ef4444';
        const svg = document.getElementById('iron-triangle-svg');
        svg.innerHTML = `
            <defs><filter id="glow"><feGaussianBlur stdDeviation="3" result="cb"/><feMerge><feMergeNode in="cb"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
            <linearGradient id="triGrad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="rgba(59,130,246,0.1)"/><stop offset="100%" stop-color="rgba(139,92,246,0.1)"/></linearGradient></defs>
            ...
            <g class="vertex-score" onclick="switchTab('tab-inv-efficiency')"><circle cx="200" cy="30" r="32" fill="${scoreColor(invScore)}" opacity="0.2" filter="url(#glow)"/><circle cx="200" cy="30" r="24" fill="${bgFill}" stroke="${scoreColor(invScore)}" stroke-width="3"/><text x="200" y="36" text-anchor="middle" fill="${scoreColor(invScore)}" font-size="14" font-weight="700">${invScore}</text><text x="200" y="8" text-anchor="middle" fill="${mutedColor}" font-size="11">庫存效率</text></g>
            <g class="vertex-score" onclick="switchTab('tab-sup-perf')"><circle cx="50" cy="330" r="32" fill="${scoreColor(supScore)}" opacity="0.2" filter="url(#glow)"/><circle cx="50" cy="330" r="24" fill="${bgFill}" stroke="${scoreColor(supScore)}" stroke-width="3"/><text x="50" y="336" text-anchor="middle" fill="${scoreColor(supScore)}" font-size="14" font-weight="700">${supScore}</text><text x="50" y="370" text-anchor="middle" fill="${mutedColor}" font-size="11">供應商績效</text></g>
            <g class="vertex-score" onclick="switchTab('tab-cost-struct')"><circle cx="350" cy="330" r="32" fill="${scoreColor(costScore)}" opacity="0.2" filter="url(#glow)"/><circle cx="350" cy="330" r="24" fill="${bgFill}" stroke="${scoreColor(costScore)}" stroke-width="3"/><text x="350" y="336" text-anchor="middle" fill="${scoreColor(costScore)}" font-size="14" font-weight="700">${costScore}</text><text x="350" y="370" text-anchor="middle" fill="${mutedColor}" font-size="11">成本控制</text></g>
        ...
        if(d.ironTriangle){
            ...
            if(ctx){window.mfgCharts['chart-triangle-trend']=new Chart(ctx,{type:'line',data:{labels:months,datasets:[
                {label:'庫存效率',data:d.ironTriangle.map(t=>t.inventoryScore),borderColor:'#3b82f6',fill:false,tension:0.4},
                {label:'供應商績效',data:d.ironTriangle.map(t=>t.supplierScore),borderColor:'#10b981',fill:false,tension:0.4},
                {label:'成本控制',data:d.ironTriangle.map(t=>t.costScore),borderColor:'#f59e0b',fill:false,tension:0.4}
        ...
        initRadarChart('chart-triangle-radar',['庫存效率','供應商績效','成本控制','生產良率','資金效率'],'當前得分',[invScore,supScore,costScore,Math.round(avgYield*100),Math.round((1-0.32)*100)]);
    };

    // ===== Page 16: 資金佔用分析 =====
    renderMap['tab-cashflow'] = () => {
        const d=currentData; const cf=d.cashFlow; if(!cf) return;
        const pos=filterByDate(d.purchaseOrders);
        document.getElementById('cashflow-kpis').innerHTML = `
            <div class="glass-panel kpi-card kpi-purple"><span class="kpi-title">資金佔用率</span><span class="kpi-value">${fmt.pct(cf.capitalTiedRate)}</span></div>
            <div class="glass-panel kpi-card kpi-success"><span class="kpi-title">資金釋放潛力</span><span class="kpi-value">${fmt.currency(cf.cashReleasePotential)}</span></div>
            <div class="glass-panel kpi-card kpi-danger"><span class="kpi-title">呆滯金額</span><span class="kpi-value">${fmt.currency(cf.staleAmount)}</span></div>
            <div class="glass-panel kpi-card kpi-warning"><span class="kpi-title">早到貨隱性成本</span><span class="kpi-value">${fmt.currency(cf.earlyDeliveryCost)}</span></div>`;
        initWaterfallChart('chart-cash-waterfall',['去年庫存','+新增採購','-生產消耗','+呆滯積壓','本期庫存'],[cf.lastYearInventory,cf.newPurchases,-cf.productionConsumption,cf.staleAccumulation,cf.currentInventory]);
        destroyChart('chart-cash-gauge');
        const ctx2=document.getElementById('chart-cash-gauge');
        if(ctx2){const rate=cf.capitalTiedRate*100;window.mfgCharts['chart-cash-gauge']=new Chart(ctx2,{type:'doughnut',data:{labels:['佔用','可用'],datasets:[{data:[rate,100-rate],backgroundColor:['#ef4444','rgba(255,255,255,0.1)'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,cutout:'75%',plugins:{legend:{display:false},datalabels:{display:false}}}});}
    ...
    renderMap['tab-risk-matrix'] = () => {
        const d=currentData; const rm=d.riskMatrix; if(!rm) return;
        ...
        document.getElementById('risk-matrix-kpis').innerHTML = `
            ...
            <div class="glass-panel kpi-card kpi-success"><span class="kpi-title">風險類別數</span><span class="kpi-value">${rm.length}</span></div>`;
        destroyChart('chart-risk-bubble');
        const ctx=document.getElementById('chart-risk-bubble');
        if(ctx){
            const sColors={critical:'rgba(239,68,68,0.7)',high:'rgba(245,158,11,0.7)',medium:'rgba(59,130,246,0.7)',low:'rgba(16,185,129,0.7)'};
            window.mfgCharts['chart-risk-bubble']=new Chart(ctx,{type:'bubble',
                data:{datasets:rm.map(r=>({label:r.category,data:[{x:r.probability*100,y:r.impact*100,r:12+r.items.length*5}],backgroundColor:sColors[r.severity]||'rgba(100,100,100,0.5)',borderColor:'transparent'}))},
                options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top'},datalabels:{display:false},tooltip:{callbacks:{label:c=>`${c.dataset.label}: 機率${c.raw.x.toFixed(0)}%, 影響${c.raw.y.toFixed(0)}%`}}},
                    scales:{x:{title:{display:true,text:'發生機率 (%)'},min:0,max:100,grid:{color:'var(--chart-grid)'}},y:{title:{display:true,text:'影響程度 (%)'},min:0,max:100,grid:{color:'var(--chart-grid)'}}}}});
        }
        ...
        if(ctx2){
            ...
            window.mfgCharts['chart-risk-trends']=new Chart(ctx2,{type:'line',
                data:{labels:tLabels,datasets:rm.map((r,i)=>({label:r.category,data:r.trend.map(v=>+(v*100).toFixed(1)),borderColor:colors[i%colors.length],backgroundColor:'transparent',tension:0.4,pointRadius:3,borderWidth:2}))},
                options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{font:{size:10}}},datalabels:{display:false}},scales:{y:{title:{display:true,text:'風險指數'},grid:{color:'var(--chart-grid)'}},x:{grid:{display:false}}}}});
        }
        const allItems=[];
        rm.forEach(r=>{r.items.forEach(item=>{allItems.push({item,category:r.category,probability:r.probability,impact:r.impact,severity:r.severity,score:Math.round(r.probability*r.impact*100),action:r.severity==='critical'?'立即建立替代供應商':'加強監控並制定應急計畫'});});});
        allItems.sort((a,b)=>b.score-a.score);
        document.getElementById('table-risk-top10').innerHTML=`<table><thead><tr><th>項目</th><th>風險類別</th><th>機率</th><th>影響</th><th>風險分數</th><th>建議行動</th></tr></thead><tbody>${allItems.slice(0,10).map(r=>`<tr><td>${r.item}</td><td>${r.category}</td><td>${fmt.pct(r.probability)}</td><td>${fmt.pct(r.impact)}</td><td><span class="status-badge ${r.severity==='critical'?'badge-danger':r.severity==='high'?'badge-warning':'badge-info'}">${r.score}</span></td><td style="font-size:0.75rem">${r.action}</td></tr>`).join('')}</tbody></table>`;
    };

    // ===== Page 18: 採購效率象限 =====
    renderMap['tab-procurement-quad'] = () => {
        const d=currentData; const pos=filterByDate(d.purchaseOrders);
        if(!pos.length) return;
        const avgCycle=pos.reduce((s,p)=>s+(p.cycleDays||p.actualLeadTime),0)/pos.length;
        const sorted=[...pos].sort((a,b)=>a.totalAmount-b.totalAmount);
        const medianAmount=sorted[Math.floor(sorted.length/2)].totalAmount;
        const xT=avgCycle,yT=medianAmount;
        const quadrants={
            strategic:pos.filter(p=>(p.cycleDays||p.actualLeadTime)>=xT&&p.totalAmount>=yT),
            inefficient:pos.filter(p=>(p.cycleDays||p.actualLeadTime)>=xT&&p.totalAmount<yT),
            fast:pos.filter(p=>(p.cycleDays||p.actualLeadTime)<xT&&p.totalAmount<yT),
            urgent:pos.filter(p=>(p.cycleDays||p.actualLeadTime)<xT&&p.totalAmount>=yT),
        };
        ...
        if(ctx){
            const qC={strategic:'#8b5cf6',inefficient:'#ef4444',fast:'#10b981',urgent:'#f59e0b'};
            const qL={strategic:'戰略採購(A)',inefficient:'效率異常(B)',fast:'快速採購(C)',urgent:'緊急採購(D)'};
            window.mfgCharts['chart-proc-scatter']=new Chart(ctx,{type:'scatter',
                data:{datasets:Object.entries(quadrants).map(([k,items])=>({label:qL[k],data:items.slice(0,50).map(p=>({x:p.cycleDays||p.actualLeadTime,y:p.totalAmount})),backgroundColor:qC[k],pointRadius:5,pointHoverRadius:7}))},
                options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top'},datalabels:{display:false},tooltip:{callbacks:{label:c=>`${c.dataset.label}: ${c.raw.x}天, ${fmt.currency(c.raw.y)}`}}},
                    scales:{x:{title:{display:true,text:'採購週期 (天)'},grid:{color:'var(--chart-grid)'}},y:{title:{display:true,text:'採購金額'},grid:{color:'var(--chart-grid)'}}}}});
        }
        document.getElementById('quadrant-stats').innerHTML=`
            ...
            <div class="glass-panel quadrant-stat"><div class="q-label" style="color:var(--danger)">B 效率異常</div><div class="q-value" style="color:#ef4444">${quadrants.inefficient.length} 筆</div><div class="q-count">小額長週期</div></div>
        ...
        document.getElementById('table-proc-anomaly').innerHTML=`<table><thead><tr><th>PO</th><th>物料</th><th>供應商</th><th>週期(天)</th><th>金額</th><th>狀態</th></tr></thead><tbody>${zoneB.map(p=>`<tr><td>${p.poNumber}</td><td>${p.materialName}</td><td>${p.supplierName}</td><td>${p.cycleDays||p.actualLeadTime}</td><td>${fmt.currency(p.totalAmount)}</td><td><span class="status-badge badge-danger">效率異常</span></td></tr>`).join('')}</tbody></table>`;
    };

    // ===== Page 19: 情境模擬器 =====
    const renderSimulator = () => {
        const d=currentData; if(!d) return;
        const pros=filterByDate(d.productionOrders); if(!pros.length) return;
        ...
        const yieldImpact=avgYield/newYield;
        const simLabor=baseLabor*yieldImpact;
        const simOH=baseOH*(1+volumeChange*-0.3);
        ...
        const baseByMonth=months.map(m=>{const mp=pros.filter(p=>p.month===m);const vol=mp.reduce((s,p)=>s+p.goodOutput,0);const cost=mp.reduce((s,p)=>s+(p.actMaterialCost+p.actLaborCost+p.actOverheadCost)*p.goodOutput,0);return vol?+(cost/vol).toFixed(2):0;});
        ...
        if(ctx1){window.mfgCharts['chart-sim-cost']=new Chart(ctx1,{type:'line',data:{labels:months,datasets:[
        ...
        if(ctx2){window.mfgCharts['chart-sim-structure']=new Chart(ctx2,{type:'bar',data:{labels:['基準','模擬'],datasets:[
        ...
        ]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top'},datalabels:{display:false}},scales:{y:{stacked:true,max:100,grid:{color:'var(--chart-grid)'}},x:{stacked:true,grid:{display:false}}}}});}
        const matD=simMat-baseMat,labD=simLabor-baseLabor,ohD=simOH-baseOH;
        initWaterfallChart('chart-sim-waterfall',['基準成本','原料變動','人工變動','費用變動','模擬成本'],[baseTotal,matD,labD,ohD,simTotal]);
    };
    renderMap['tab-whatif'] = () => { renderSimulator(); };
    ['sim-price','sim-yield','sim-volume'].forEach(id=>{document.getElementById(id).addEventListener('input',()=>{const ap=document.querySelector('.tab-pane.active');if(ap&&ap.id==='tab-whatif')renderSimulator();});});
    ...
    renderMap['tab-exec-cockpit'] = () => {
        ...
        document.getElementById('cockpit-traffic').innerHTML=`<span style="font-weight:600;font-size:0.85rem;">狀態摘要</span>
        ...
        document.getElementById('cockpit-gauges').innerHTML=gauges.map(g=>`
            <div class="glass-panel gauge-widget"><canvas id="gauge-${g.id}" width="200" height="200"></canvas>
            ...
            <div class="gauge-sparkline">${g.sparkData.map(v=>`<div class="spark-bar" style="height:${v*0.24}px;background:${g.color}"></div>`).join('')}</div></div>`).join('');
        gauges.forEach(g=>{
            destroyChart('gauge-'+g.id);
            const ctx=document.getElementById('gauge-'+g.id);if(!ctx)return;
            const gc=g.value>=85?'#10b981':g.value>=70?'#f59e0b':'#ef4444';
            window.mfgCharts['gauge-'+g.id]=new Chart(ctx,{type:'doughnut',data:{datasets:[{data:[g.value,100-g.value],backgroundColor:[gc,'rgba(255,255,255,0.06)'],borderWidth:0}]},
                options:{responsive:true,maintainAspectRatio:true,rotation:-90,circumference:180,cutout:'80%',plugins:{legend:{display:false},datalabels:{display:false},tooltip:{enabled:false}}}});
        });
        document.getElementById('cockpit-insights').innerHTML=[
            ...
            {icon:'🚚',title:'供應商預警',text:`${d.suppliers.filter(s=>s.tier==='At-Risk').length} 家供應商處於 At-Risk 等級，建議啟動供應商輔導計畫或尋找備援來源。`},
            {icon:'💰',title:'成本觀察',text:`本期成本差異率為 ${fmt.pct(costVarRate)}，主要來自材料價格波動。建議加強長約比例，鎖定關鍵料件價格。`},
            {icon:'🏭',title:'良率提升',text:`整體良率 ${fmt.pct(avgYield)}，${pros.filter(p=>p.yieldRate<0.95).length} 筆工單良率低於 95%，建議優先改善製程穩定性。`},
        ].map(ins=>`<div class="glass-panel insight-card"><div class="insight-icon">${ins.icon}</div><div class="insight-title">${ins.title}</div><div class="insight-text">${ins.text}</div></div>`).join('');
    };

    // ===== Page 21: 價值流圖 =====
    renderMap['tab-vsm'] = () => {
        const d=currentData;const vs=d.valueStream;if(!vs) return;
        ...
        document.getElementById('vsm-flow').innerHTML=vs.map((node,i)=>{
            const nh=`<div class="vsm-node ${node.bottleneck?'bottleneck':''}"><div class="node-title">${node.name}</div><div class="node-metric">週期: <b>${node.cycleTime}天</b></div><div class="node-metric">等待: <b>${node.waitTime}天</b></div><div class="node-metric">增值率: <b>${(node.valueAddRatio*100).toFixed(0)}%</b></div></div>`;
        ...
        document.getElementById('vsm-timeline').innerHTML=`
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;"><span style="font-size:0.85rem;font-weight:600;">前置時間分解</span><span style="font-size:0.8rem;color:var(--text-secondary)">總計 ${totalLead.toFixed(1)} 天</span></div>
            <div class="vsm-timeline-bar">${vs.map((n,i)=>`<div class="segment" style="width:${((n.cycleTime+n.waitTime)/totalLead*100).toFixed(1)}%;background:${colors[i%colors.length]}" title="${n.name}: ${(n.cycleTime+n.waitTime).toFixed(1)}天"></div>`).join('')}</div>
            <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:8px;">${vs.map((n,i)=>`<span style="font-size:0.7rem;color:var(--text-secondary);display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;border-radius:2px;background:${colors[i%colors.length]};display:inline-block"></span>${n.name}</span>`).join('')}</div>
            <div class="vsm-efficiency"><div class="stat"><div class="stat-value" style="color:var(--success)">${fmt.pct(valueAddPct)}</div><div class="stat-label">增值比例</div></div><div class="stat"><div class="stat-value" style="color:var(--danger)">${fmt.pct(wastePct)}</div><div class="stat-label">浪費比例</div></div></div>`;
        initBarChart('chart-vsm-cycle',vs.map(n=>n.name),'天數',vs.map(n=>n.cycleTime+n.waitTime),false,vs.map(n=>n.bottleneck?'#ef4444':'#3b82f6'));
        destroyChart('chart-vsm-ratio');
        const ctx=document.getElementById('chart-vsm-ratio');
        if(ctx){window.mfgCharts['chart-vsm-ratio']=new Chart(ctx,{type:'bar',data:{labels:vs.map(n=>n.name),datasets:[
        ...
        ]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top'},datalabels:{display:false}},scales:{y:{stacked:true,grid:{color:'var(--chart-grid)'}},x:{stacked:true,grid:{display:false}}}}});}
    };

    // ===== Page 22: 標竿對比 =====
    renderMap['tab-benchmark'] = () => {
        const d=currentData;const bm=d.benchmark;if(!bm) return;
        ...
        document.getElementById('benchmark-kpis').innerHTML=`
            <div class="glass-panel kpi-card"><span class="kpi-title">平均標竿差距</span><span class="kpi-value">${avgGap.toFixed(1)} pts</span></div>
            ...
            <div class="glass-panel kpi-card kpi-purple"><span class="kpi-title">最大差距維度</span><span class="kpi-value">${bm.dimensions[bm.dimensions.reduce((mi,_,i)=>(bm.industryBest[i]-bm.company[i])>(bm.industryBest[mi]-bm.company[mi])?i:mi,0)]}</span></div>`;
        destroyChart('chart-bench-radar');
        const ctx=document.getElementById('chart-bench-radar');
        if(ctx){window.mfgCharts['chart-bench-radar']=new Chart(ctx,{type:'radar',
            data:{labels:bm.dimensions,datasets:[
                ...
                {label:'行業平均',data:bm.industryAvg,borderColor:'#f59e0b',backgroundColor:'rgba(245,158,11,0.08)',pointBackgroundColor:'#f59e0b',borderWidth:2,borderDash:[5,5]},
                {label:'行業標竿',data:bm.industryBest,borderColor:'#10b981',backgroundColor:'rgba(16,185,129,0.08)',pointBackgroundColor:'#10b981',borderWidth:2}
            ]},options:{responsive:true,maintainAspectRatio:false,scales:{r:{beginAtZero:true,max:100,grid:{color:'rgba(255,255,255,0.08)'},angleLines:{color:'rgba(255,255,255,0.08)'}}},plugins:{legend:{position:'bottom'},datalabels:{display:false}}}});}
        destroyChart('chart-bench-gap');
        const ctx2=document.getElementById('chart-bench-gap');
        if(ctx2){const gaps=bm.dimensions.map((_,i)=>bm.industryBest[i]-bm.company[i]);
            window.mfgCharts['chart-bench-gap']=new Chart(ctx2,{type:'bar',
                data:{labels:bm.dimensions,datasets:[{label:'公司',data:bm.company,backgroundColor:'#3b82f6',borderRadius:4},{label:'差距到標竿',data:gaps,backgroundColor:'rgba(239,68,68,0.5)',borderRadius:4}]},
                options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top'},datalabels:{display:false}},scales:{x:{stacked:true,grid:{color:'var(--chart-grid)'}},y:{stacked:true,grid:{display:false}}}}});}
        const roadmap=bm.dimensions.map((_,i)=>({dimension:bm.dimensions[i],current:bm.company[i],target:bm.industryBest[i],gap:bm.industryBest[i]-bm.company[i],
        ...
        document.getElementById('table-bench-roadmap').innerHTML=`<table><thead><tr><th>維度</th><th>現況</th><th>標竿</th><th>差距</th><th>優先級</th><th>改善方向</th></tr></thead><tbody>${roadmap.map(r=>`<tr><td>${r.dimension}</td><td>${r.current}</td><td>${r.target}</td><td>${r.gap}</td><td><span class="status-badge ${r.priority==='高'?'badge-danger':r.priority==='中'?'badge-warning':'badge-success'}">${r.priority}</span></td><td style="font-size:0.75rem">${r.action}</td></tr>`).join('')}</tbody></table>`;
    };
...
v3 = v3.replace(init_block, new_js_block)

# Also update the btn-reset handler
v3 = v3.replace(
...
v3 = v3.replace("a.download = 'manufacturing_data_v2.json';", "a.download = 'manufacturing_data_v3.json';")
v3 = v3.replace("XLSX.writeFile(wb, 'Manufacturing_Data_V2.xlsx');", "XLSX.writeFile(wb, 'Manufacturing_Data_V3.xlsx');")
